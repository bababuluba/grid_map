...
bool GridMapOctomapConverter::fromOctomap(const octomap::ColorOcTree& octomap,
...
//    return false;
  ...
  octomap::ColorOcTree octomapCopy(octomap);
  ...
  do {
    ...
    for (octomap::ColorOcTree::iterator it = octomapCopy.begin(); it != octomapCopy.end(); ++it) {
  ...
  for(octomap::ColorOcTree::leaf_bbx_iterator it = octomapCopy.begin_leafs_bbx(minBbx, maxBbx),
